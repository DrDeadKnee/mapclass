...
1. **Build a dataset of pixel-label pairs.** Sources:
    - **Synthetic illustrated maps** — use an agentic model to generate maps via Azgaar's Fantasy Map Generator and a custom Pillow-based renderer. Because the agent controls asset placement, ground truth pixel annotations (land cover + topography) are generated automatically in the same pass. Multiple rendering styles are used to maximise visual diversity. Azgaar's biome taxonomy is mapped to the canonical 9-class land cover taxonomy; cropland and built-up (absent from fantasy maps) are added via the Pillow renderer and up-weighted in training relative to satellite-derived examples. Topography labels are derived from Azgaar's normalised heightmap using fixed thresholds (flat ≤ 20, hilly 20–55, mountainous > 55); calibration against SRTM statistics is left as future work.
